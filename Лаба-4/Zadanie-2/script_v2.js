...
orderList.push(
    ...
    new Order(3, "Тестер", "Тестеров", "tester@top.com", "г.Тест, ул. Тестеров, д. -004", 5)
);
...
productList.forEach(async (product) => {
    try {
        const payloadProduct = await loadProducts(product);
...
async function loadProducts(product){
    if (!product.getImgPath || !product.getName || !product.getType || !product.getPrice){
        throw new Error(`Товар ${product.getId} не может быть добавлен!`);
    }
    ...
    return `Товар ${product.getId} успешно добавлен`;
};
...
let loadOrderPromise = function(order){
    return new Promise((resolve, reject) => {
        loadOrder(order, (error, payloadOrder) => {
            if (error) reject (error)
            else resolve(payloadOrder);
        });
    });
};
...
orderList.forEach((order) => {
    loadOrderPromise(order)
        .then(payloadOrder => console.log(payloadOrder))
