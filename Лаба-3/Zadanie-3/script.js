...
try{
    ...
    console.log("Ошибка при выводе товаров!");
}
finally {
    document.getElementById("ProductCount").textContent += ` (${countProducts} шт.)`;
}
...
//Работа с файлом
const fileReadButton = document.getElementById("fileRead");

fileReadButton.addEventListener("click", function(event){
    event.preventDefault();
    document.getElementById("fileWorkResult").textContent = "";

    try{
        const fileTemp = document.getElementById("fileGetInput");
        if (!fileTemp.files.length){
            throw new Error("Файл не выбран!");
        }
        
        const file = fileTemp.files[0];
        
        if (file.type != "text/plain"){
            throw new Error("Выбран формат не .txt");
        }

        const reader = new FileReader();
    
        reader.onload = function(e) {
            document.getElementById("fileWorkResult").textContent = e.target.result;
        };
        ...
        document.getElementById("fileWorkResult").textContent = error.message;
    }
});